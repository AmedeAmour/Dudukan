-- Single-result Supabase/PostgreSQL inventory.
-- Run manually or through the Supabase SQL Editor. Read-only.
-- Returns one JSON object in one row for easier copy/export from the dashboard.
...
select jsonb_pretty(
  jsonb_build_object(
    ...
    'migration_history', coalesce((
      select jsonb_agg(to_jsonb(x) order by x.version)
      from (
        select *
