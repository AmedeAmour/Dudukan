...
export default async function handler(req, res) {
  ...
  try {
    ...
    const currency = transaction?.currency?.iso || transaction?.currency || PLAN.currency;
    const isApproved = eventName === 'transaction.approved' || status === 'approved';
    const isExpectedPlan = amount === PLAN.amount && currency === PLAN.currency && metadata.plan_code === PLAN.code;
